...
function Hp(figure, permissionRoles) {
  if (figure.indexOf('admin') >= 0) return true
  if (!permissionRoles) return true
  return figure.some(role => permissionRoles.indexOf(role) >= 0)
}
...
router.beforeEach((to, from, next) => {
  ...
  if (getToken('Token')) {
    if(to.path === '/login') {
      ...
      if (store.getters.figure.length === 0) {
        let token = getToken('Token');
        getUserInfo({"token":token}).then().then(res => { // 根据token拉取用户信息
          let userList = res.data.userList;
          store.commit("SET_ROLES",userList.figure);
          store.commit("SET_NAME",userList.name);
          store.commit("SET_AVATAR",userList.avatar);
          store.dispatch('GenerateRoutes', { "figure":userList.figure }).then(() => { // 根据roles权限生成可访问的路由表
        ...
        if (Hp(store.getters.figure, to.meta.figure)) {
          next()//
        } else {
...
/**
  本系统权限逻辑分析：
  1、路由对象区分权限路由对象和非权限路由对象；初始化时，将非权限路由对象赋值给Router;同时设置权限路由中的meta对象，如:meta:{figure:['admin','editor']}
     表示该roles所拥有的路由权限;
   ...
   1.1、用户已经登录成功过，并从cookie中拿到了token值;
     ...
     1.1.1、用户访问非登录页面,需要根据用户是否有roles信息，进行不同的业务逻辑,如下：
        (1)、初始情况下,用户roles信息为空;
            ...
            2.通过store.dispatch('GenerateRoutes', { figure })去重新过滤和生成路由,通过router.addRoutes()合并路由表;
            3.如果在获取用户信息接口时出现错误，则调取store.dispatch('LogOut')接口,返回到login页面;

        (2)、用户已经拥有roles信息；
            1.点击页面路由，通过roles权限判断 Hp()。如果用户有该路由权限，直接跳转对应的页面;如果没有权限，则跳转至401提示页面;
